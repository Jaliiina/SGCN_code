...
# 避免多线程问题，将数据路径设置为英文路径
os.makedirs('data', exist_ok=True)
dataset = Planetoid(root='data/Pubmed', name='Pubmed')
data = dataset[0]

# 手动创建 processed 目录避免后续报错
os.makedirs(dataset.processed_dir, exist_ok=True)

# 设置 num_workers=0 避免 Windows 下子进程崩溃问题
...
subgraph_loader = NeighborLoader(data, num_neighbors=[-1], batch_size=128,
...
class Net(torch.nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.convs = ModuleList([
            SAGEConv(in_channels, 32),
            SAGEConv(32, out_channels)])
