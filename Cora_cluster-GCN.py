...
cluster_data = ClusterData(data, num_parts=20, recursive=False,
                           save_dir=dataset.processed_dir)
train_loader = ClusterLoader(cluster_data, batch_size=5, shuffle=True,
                             num_workers=0)

subgraph_loader = NeighborLoader(data, num_neighbors=[-1], batch_size=64,
...
class Net(torch.nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.convs = ModuleList([
            SAGEConv(in_channels, 16),
            SAGEConv(16, out_channels)])
