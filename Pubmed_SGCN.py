...
def build_snowball_subgraph(seed_nodes, adj, max_nodes=300):
...
def train_on_subgraph(sub_nodes, data, base_params=None):
    ...
    model = GCN(sub_x.size(1), 32, dataset.num_classes).to(device)
    if base_params:
        model.load_state_dict(base_params)

    optimizer = torch.optim.Adam(model.parameters(), lr=0.01, weight_decay=5e-4)
    for epoch in range(5):  # train each subgraph for 5 epochs
        model.train()
...
# ========== Main loop: bootstrapped training ==========
...
for outer in range(50):
    ...
    for inner in range(20):
        ...
        print(f"Sub-model {inner+1}/20 finished | val_acc={acc_sub:.4f} | time={duration:.4f}s")

    # Weighted averaging
    base_params = average_state_dicts(sub_models, sub_weights)

    # Validate aggregated model
    model_eval = GCN(dataset.num_node_features, 32, dataset.num_classes).to(device)
...
final_model = GCN(dataset.num_node_features, 32, dataset.num_classes).to(device)
